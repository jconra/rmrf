...
import { initFire, fireBurst, fireWreck, tickFire, drawFire, fireStatus } from './Fire.js?v=11';
...
// HOW FAR AHEAD THE AI AIMS, PER GUN (task #46). One global 0.7 was applied to every weapon in the
// game, and the weapons are not comparable: PROJ_SPEED is [85, 0, 32, 115], so the Valkyrie's missile
// is in the air for ~3s at 100u while the Lurcher's tracer takes 1.2s and the Firebrat is hitscan.
// Under-leading by 30% costs a tracer a few units and costs a missile most of a hull-length, so the
// slow weapons need the full solution and the fast ones can stay loose enough to be dodged.
// Indexed by soundIndex: 0 Lurcher tracer · 1 Firebrat (hitscan — unused, leadAim returns early)
// · 2 Valkyrie missile · 3 Jotun slug (115 u/s plus a 0.9s charge, so it aims furthest ahead in time).
const AIM_LEAD_BY_GUN = [0.7, 0.7, 1.0, 0.95];
const AIM_LEAD       = 0.7;   // fallback for any caller without a gun index
// DIFFICULTY (?difficulty=easy|medium|hard, default medium). Two dials, because they miss in
// different ways: `lead` is how much of the intercept solution the gunner actually applies — an easy
// opponent shoots behind a moving target — and `jitter` scales the existing per-personality aim
// noise. Hard does not get MORE than the true solution; it gets the true solution reliably, which is
// already a large step up from 0.7.
const AIM_DIFFICULTY = { easy: { lead: 0.55, jitter: 2.2 }, medium: { lead: 1, jitter: 1 }, hard: { lead: 1, jitter: 0.4 } };
let aimDiff = AIM_DIFFICULTY.medium;
// ?oldlead restores the single global 0.7 with no difficulty scaling — the exact pre-2026-08-18
// behaviour, so the gate has a real control arm inside ONE build rather than diffing two trees.
let LEAD_FIX = true;
...
function leadAim(shooterPos, enemy, soundIndex, jitter = 0) {
  ...
  const base = LEAD_FIX && AIM_LEAD_BY_GUN[soundIndex] != null ? AIM_LEAD_BY_GUN[soundIndex] : AIM_LEAD;
  const dl = LEAD_FIX ? aimDiff.lead : 1, dj = LEAD_FIX ? aimDiff.jitter : 1;
  const lead = base * dl * (1 + (Math.random() - 0.5) * jitter * dj);
...
// DIFFICULTY — ?difficulty=easy|medium|hard (task #46). Also RR.setDifficulty() for the lab, and it
// is read live so a running match can be re-tuned without a reload.
{ const d = (QS.get('difficulty') || '').toLowerCase(); if (AIM_DIFFICULTY[d]) aimDiff = AIM_DIFFICULTY[d]; }
LEAD_FIX = !QS.has('oldlead');
    ...
    if (!this.unit || this.unit.dead) {
      if (this.unit && this.unit.dead) {
        ...
        if (this.unit.type === 'firebrat' && runStep === 'capture') {
          const tt = this.targetTeam();
          // WHAT ACTUALLY KILLED THIS RUNNER (task #44, defect 2). This asked whether the enemy owns
          // a vehicle ANYWHERE ON THE MAP, which is not the same question and answered it wrong 14
          // times out of 15: measured over 8 seeds, 14 of 15 runner deaths took the interceptor
          // branch while the damage ledger said 14 of 14 were PURE TOWER kills. The branch decides
          // whether the answer is "clear their defenders" or "go silence that gun", so getting it
          // wrong means the siege boost — the only thing that removes the gun that is actually
          // killing runners — essentially never fired.
          //
          // veh._dmgBy has tallied {turret, vehicle, tree, other} per unit the whole time; nothing
          // had to be built, only asked. The old comment defended the map-wide proxy as a "better
          // signal than the death's damage-split, which mislabels a mixed tower+vehicle kill" — but
          // there were no mixed kills in the sample, so it avoided a case that does not occur and
          // got the real one wrong nearly every time. Ties and empty ledgers fall back to the proxy,
          // which is the only situation the old reasoning actually described.
          const by = this.unit._dmgBy;
          const enemyHasUnits = (KILLER_FIX && by && (by.vehicle || by.turret))
            ? by.vehicle > by.turret
            : combatants.some(o => !o.dead && o.team === tt && !vehicleHidden(o));
